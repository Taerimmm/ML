...
labels_df = pd.read_csv('./dacon3/data/dirty_mnist_2nd_answer.csv')[:]
...
for fold in range(5):
    ...
    for epoch in range(epochs):
        ...
        if np.mean(valid_batch_accuracy) > valid_best_accuracy:
            torch.save(model.state_dict(), './dacon3/data/EfficientNetB0-fold{}.pt'.format(fold))
            valid_best_accuracy = np.mean(valid_batch_accuracy)
        print('fold : {}\tepoch : {:02d}\ttrain_accuracy / loss : {:.5f}\tvalid_accuracy / loss : {:.5f} / {:.5f} \ttime : {:.0f}'.format(fold+1,
                                                                                                                                   epoch+1,
