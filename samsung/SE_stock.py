...
# X, Y 나누기
x_data = stock.iloc[:,[0,1,2,3,5,6]]
y_data = stock['종가']

# print(x_data.columns)  # ['시가', '고가', '저가', '종가', '거래량', '금액(백만)']
...
print(x_data[:-1].shape)        # (632, 28, 6)
print(y_target.shape)           # (632,)
...
np.save('./samsung/etc/stock_x_data.npy', arr=x_data)
np.save('./samsung/etc/stock_x_train.npy', arr=x_train)
np.save('./samsung/etc/stock_x_test.npy', arr=x_test)
np.save('./samsung/etc/stock_y_train.npy', arr=y_train)
np.save('./samsung/etc/stock_y_test.npy', arr=y_test)
...
modelpath = './samsung/etc/modelcheckpoint/stock_model.hdf5'
es = EarlyStopping(monitor='val_loss', patience=200, mode='auto')
cp = ModelCheckpoint(filepath=modelpath, monitor='val_loss', save_best_only=True, mode='auto')
model.fit(x_train, y_train, epochs=10000, batch_size=4, validation_split=0.2, verbose=2, callbacks=[es,cp], shuffle=False)

model.save('./samsung/etc/stock_model.h5')
