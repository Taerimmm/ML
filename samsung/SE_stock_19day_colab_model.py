...
np.save('./samsung/etc/19day_data_1.npy', arr=x1_data)
np.save('./samsung/etc/19day_data_2.npy', arr=x2_data)
...
y_target = y_data[size_x-size_y+1:]  # <---- 아마 다를거임 -1 이면 -size_y+1 로 해야함
...
model.fit([x1_train,x2_train], y_train, epochs=1, batch_size=4, validation_data=([x1_val,x2_val], y_val), verbose=2, callbacks=[es,cp])

model.save('./samsung/etc/19day_stock_model_2.h5')
...
# "2021-01-18"의 시가는 [[         ]] 로 예측 됩니다.
# "2021-01-19"의 시가는 [[         ]] 로 예측 됩니다.
