...
print(stock.iloc[-666:-662,:])

print(stock.iloc[:-664,:])

stock.iloc[:-664,0:4] = stock.iloc[:-664,0:4]/50.       # 시가, 고가, 저가, 종가 1/50 배 (액면분할)
stock.iloc[:-664,5] = stock.iloc[:-664,5]*50.           # 거래량 50배
...
print(kos.iloc[-667:-664,:]) # <- stock에 없는 날짜
kos.drop(['2018-04-30','2018-05-02','2018-05-03'], inplace=True)
print(stock.iloc[-666:-662,:])

print(kos.shape)            # (1085, 15)
...
print(x1_data.shape)            # (1076, 10, 7)
print(x2_data.shape)            # (1076, 10, 8)

size_y = 2 
y_data = split_x(pd.DataFrame(y_data),size_y)[-1085:]
print(y_data)

print(y_data.shape)             # (1085, 2, 1)

y_target = y_data[size_x:]

print(x1_data[:-1].shape)        # (1075, 10, 7)
print(x2_data[:-1].shape)        # (1075, 10, 8)
print(y_target.shape)            # (1075, 2, 1)
...
layer1 = GRU(64, activation='relu')(input1)
layer1 = Dropout(0.2)(layer1)
layer1 = Dense(256, activation='relu')(layer1)

input2 = Input(shape=(x2_data.shape[1], x2_data.shape[2]))
layer2 = Conv1D(32, 3, padding='same', strides=1, activation='relu')(input2)
...
merge = Dense(1024)(merge)
merge = Dense(512)(merge)
merge = Dense(256)(merge)
merge = Dense(128)(merge)
merge = Dense(64)(merge)
merge = Dense(32)(merge)
output1 = Dense(2)(merge)
...
model.fit([x1_train, x2_train], y_train, epochs=20000, batch_size=1, validation_data=([x1_val, x2_val], y_val), verbose=2, callbacks=[es, cp])
...
print('2021-01-18 의 시가는', a[0][0])
print('2021-01-19 의 시가는', a[0][1])
