...
def my_model():
    ...
    layer1 = Conv1D(128,2,activation='relu',padding='same',strides=1)(layer1)
    layer1 = LSTM(64, activation='relu')(layer1)
...
reduce_lr = ReduceLROnPlateau(monitor='val_loss', patience=5, factor=0.25, verbose=1)
...
for a in range(48):
    ...
    for i, j in enumerate(quantiles): # Day 7
        ...
        model.fit(x_train, y1_train, epochs=500, batch_size=512, validation_data=(x_val, y1_val), verbose=2, callbacks=[es,cp,reduce_lr])
    ...
    for i, j in enumerate(quantiles): # Day 8
        ...
        cp = ModelCheckpoint(filepath=modelpath, monitor='val_loss', save_best_only=True, mode='auto')
        model.fit(x_train, y2_train, epochs=500, batch_size=512, validation_data=(x_val, y1_val), verbose=2, callbacks=[es,cp,reduce_lr])
...
submission.to_csv('./dacon/data/sample_submission_final_version4.csv')
