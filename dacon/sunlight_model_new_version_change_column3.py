...
for i, j in enumerate(quantiles): # Day 8
    ...
    cp = ModelCheckpoint(filepath=modelpath, monitor='val_loss', save_best_only=True, mode='auto')
    reduce_lr = ReduceLROnPlateau(monitor='val_loss', patience=10, factor=0.25, verbose=1)
    model.fit(x_train, y2_train, epochs=5000, batch_size=32, validation_data=(x_val, y2_val), verbose=2, callbacks=[es,cp,reduce_lr])
...
submission.to_csv('./dacon/data/sample_submission_new_version3.csv')
