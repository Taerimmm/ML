...
train_data = preprocess_data(add_features2(add_features(train_data)))
print(train_data.iloc[:48])
print(train_data.shape)     # (52464, 9)
...
for i in range(81):
    ...
    temp = preprocess_data(add_features2(add_features(temp)), is_train=False)
...
output1 = Dense(1,activation='relu')(layer1)
...
for i, j in enumerate(quantiles): # Day 7
    ...
    model.fit(x_train, y1_train, epochs=10000, batch_size=32, validation_data=(x_val, y1_val), verbose=2, callbacks=[es,cp,reduce_lr])
...
for i, j in enumerate(quantiles): # Day 8
    ...
    cp = ModelCheckpoint(filepath=modelpath, monitor='val_loss', save_best_only=True, mode='auto')
    reduce_lr = ReduceLROnPlateau(monitor='val_loss', patience=10, factor=0.25, verbose=1)
    model.fit(x_train, y2_train, epochs=10000, batch_size=32, validation_data=(x_val, y2_val), verbose=2, callbacks=[es,cp,reduce_lr])
...
submission.to_csv('./dacon/data/sample_submission_new_version2.csv')
